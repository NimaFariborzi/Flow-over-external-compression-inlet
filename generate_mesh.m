...
% Grid spacing
d_xi = 0.1;  % Grid spacing in xi
d_et = 0.05; % Grid spacing in eta
...
X2 = rescale(XI2,x1_dist,x1_dist + x2_dist);
Y2 = zeros(size(ET2));

y_max = y1_dist;
for i = 1:length(xi2)
    x = X2(i,1);
    y_min = x1_dist*tand(delta1) + (x - x1_dist)*tand(delta2);
...
X3 = rescale(XI3,x1_dist + x2_dist,x1_dist + x2_dist + x3_dist);
Y3 = zeros(size(ET3));
...
X5 = rescale(XI5,x1_dist,x1_dist + x2_dist);
Y5 = zeros(size(ET5));
...
X6 = rescale(XI6,x1_dist + x2_dist,x1_dist + x2_dist + x3_dist);
Y6 = zeros(size(ET6));
